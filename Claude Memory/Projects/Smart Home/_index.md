...
## Status
- **2026-08-31: LIVE REGISTRY AUDIT — four automations are enabled and cannot work; there is no bedroom light entity at all.** First `curl /api/states` evidence this project has had, run from the Fold. **(a) Four silent-failure automations:** `bedroom_enter` + `bedroom_empty` fire on `binary_sensor.bedroom_bedroom_presence` (**absent from the registry**) at `light.bedroom_light` (**absent**) — they have never been able to fire; `ai_cam_person_detected_living_room_light` + `..._cleared_...` depend on `binary_sensor.ai_cam_person_occupancy` and `light.living_room_light`, both unavailable. All four sit at `state: on` looking healthy. **(b) No bedroom light exists** — ten `light.*` entities, six unavailable, none of them a bedroom light; `light.bedroom`, `left/right/stairs_smart_bulb`, `lounge_lights`, `rgbic_tv_backlight`, `kitchen_2` referenced in vault docs do not exist either. **(c) Bedroom presence radar down** — `bedroom_espectre_status` off, ~30 entities unavailable. **(d) ⚠️ CORRECTION — AI CAM 2 is ALIVE**, not offline as recorded earlier today: uptime 3h17m, `192.168.0.201`, −60 dBm, ESPHome 2026.8.1, MAC `28:84:85:49:86:70` — **confirming the standing hypothesis** that board #2 is the `…:86:70` unit. The "offline" call came from misreading a screenshot column; **no USB reflash needed.** **(e) Entity IDs settled** — AI CAM 2 carries TWO prefixes at once (`ai_cam_2_*` and `landing_ai_cam_2_*`), matching neither its config nor its display name; the `landing_` half is a fossil of its original room, which also resolves the recorded area conflict. **(f) Two further defects:** AI CAM 2's camera is powered down (`switch.landing_ai_cam_2_camera_power_down` = on), and Frigate is down entirely (every Frigate entity unavailable while the ESPHome camera is fine) — **so this index's "live in Frigate with recording + person detection" claim is currently false.** See [[diagnostics/2026-08-31-bedroom-entity-audit]]; rebuild blocked on one answer — what is the bedroom light?
- **2026-08-30: Supervisor log triaged — 2 defects found, 1 fixable over the API and now automated, 1 needs the host shell.** From a 100-line supervisor tail (02:50–02:53 BST). **(a) Host log subsystem broken:** `HostLogError: Could not get a list of boot IDs from systemd-journal-gatewayd`, twice in 27s, each a 20s aiohttp timeout; Supervisor logs `Could not get /boots …, using fallback` and the legacy full-journal scan never completes. Breaks the **boot selector on Settings → System → Logs**; no evidence in the tail of Core/add-ons/satellites affected. Two candidate causes needing opposite fixes — journal too large to scan (favoured: one traceback stalls *mid-body*, not at connect) vs gatewayd wedged — **not yet distinguished, needs the LAN probe**. **(b) ESPHome Device Builder (beta) carries two options its schema dropped** (`use_new_device_builder` — the retired 2026.5.0 beta toggle — and `status_use_ping`), warned on every options load; noise only, but permanent noise. **Built:** `Assistant Core/ha-diagnostics/ha-supervisor-fix.mjs` — companion to `ha-doctor` covering the Supervisor layer it explicitly does not; times a cheap tail vs the boot listing separately to return `healthy`/`boots-marginal`/`journal-too-large`/`gatewayd-down` with the matching host-shell repair, and strips stale option keys from *any* add-on over the API (dry-run default, removes keys only, never edits a value, keeps a verbatim before-image). **46/46 offline assertions**, incl. end-to-end against a mock Supervisor; the suite caught a real `Authorization`-header-drop bug before it shipped. **Nothing has touched the hub — the on-hub run is the open gate.** See [[diagnostics/2026-08-30-supervisor-log-triage]].
