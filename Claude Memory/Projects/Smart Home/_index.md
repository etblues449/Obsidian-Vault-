...
- **⚠️ microWakeWord regression (2026-08-02 live evidence):** on the current ai_cam build (fw 2026.7.3 built 2026-07-30 00:29, hash `0x8b9a6799`) both Wake-word selects are **unavailable** — "Hey Jarvis" hands-free is NOT active; the Assist satellite itself works (Responding→Playing in the activity log, push-button voice OK). Live MAC = `28:84:85:49:83:C8` (handoff recorded `...:86:70` — reconcile which board is which). "Related" on the device page is empty — no automations use the AI Cam yet. See [[sessions/2026-08-02]].
