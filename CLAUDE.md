...
| 2026-08-02 | Engine repaired | `runner.mjs`, all 5 workflows, `jarvis.js` ×2 | Exact-hour guard meant NO scheduled run ever wrote output; replaced with period idempotency. Capture Router (Skill 2) built. Root-`Inbox` fallback fixed. |
| 2026-08-02 | Recovered 8 deleted files | `.github/workflows/`, `repos-manifest.json`, `Work/setup-repos.sh`, `JARVIS-Carousel/.gitignore` | Second occurrence of a stale obsidian-git sync committing absences as deletions. Cause still unfixed — writing client unidentified. |
| 2026-08-02 | Absorbed the external runbook | §A–D below | A separately-produced `claude.md` + health check were largely contradicted by observation; the reusable parts were folded in here rather than kept as a second file that would collide with this one on Windows/Android. |

---

## A. The five layers

Shorthand used throughout the vault and in session notes.

| Layer | What | Where | State (2026-08-02, verified) |
|---|---|---|---|
| **A** | Obsidian-native brain — Capture (Alt+J), Ask (Alt+A), Digest (Alt+D) | QuickAdd scripts on the Fold, Claude API | Live |
| **B** | Voice agent — stateless web chat/voice | `jarvis-voice-lovat.vercel.app`, Groq + Web Speech | Live, £0 |
| **C** | Skill engine — Morning Brief, Connection Finder, Weekly Synthesis, Pattern Detector | GitHub Actions + Groq → commits to `master` | Engine repaired 2026-08-02. **No scheduled run had ever written output before that.** |
| **D** | Capture pipeline — thought → vault | Tasker → n8n webhook → `JARVIS/Inbox/` | Router built (Skill 2). **Phone leg still on paid n8n; Tasker variable bug still open.** |
| **E** | Smart home — hub, nodes, cameras, voice satellites | HA Green `192.168.0.200` + ESP32 | Live. 8 automations (not ~19), core on a beta, 1 of 4 voice satellites online. |

`jarvis-core` (the on-device agent: voice terminal, web app on `:8737`, 13 tools,
rails) is a **separate repo** — `etblues449/jarvis-core`, branch `main`. Its spec is
`AGENT.md` there; `JARVIS/HANDOFF.md` here is the operational companion.

## B. Command reference

```bash
# Fold 7 (Termux)
cd ~/jarvis-core && node jarvis-voice.mjs     # voice terminal
cd ~/jarvis-core && node jarvis.mjs           # text terminal
sh ~/jarvis-core/scripts/jarvis-launch.sh     # web app, one tap (starts it if down)
...
node "Assistant Core/jarvis-skills/runner.mjs" --skill=capture-router      # route captures
node "Assistant Core/jarvis-skills/runner.mjs" --skill=morning-brief --force --dry-run
```
...
| Symptom | First check |
|---|---|
| A skill produced nothing | Actions log. `reason=already-done` is normal; anything else is not. Never assume a green run wrote something. |
| Capture didn't arrive | Leg order: phone variable → transport → vault. Log the Tasker variable *immediately before* the HTTP action. |
| Web app refuses to connect | Port is **8737** — `localhost` alone is port 80. Then `scripts/jarvis-serve.sh`. |
| Files vanished from `master` | A stale client committed absences as deletions. Find the clone that authored the commit; do not just restore. |
| HA entity missing | The live registry wins over any vault note. Re-run `ha-doctor`. |

## D. Reading any status claim in this vault

<important if="about to trust a ✅ / LIVE / DEPLOYED claim in a vault note">
Three states get conflated constantly: **documented**, **merged**, **running**.
On 2026-08-02 the skill engine was documented as live, merged, and producing daily
output — and had never once produced output on a schedule. Every artefact came from
a manual run.

Before repeating a status claim, name the observation behind it: a job log, a file
on `master`, a test you ran. "Built" is also not "used" — Tier 4 memory is wired
end-to-end and has never stored a single fact.

Numbers deserve the same suspicion. Uptime percentages and latency percentiles have
appeared in JARVIS documents for a system with no telemetry that could produce them.
A gap prompts a check; a fabricated number does not.
</important>
